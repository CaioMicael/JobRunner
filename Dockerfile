...
WORKDIR /src
COPY ["JobRunner.csproj", "."]
RUN dotnet restore "./JobRunner.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./JobRunner.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Esta fase é usada para publicar o projeto de serviço a ser copiado para a fase final
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./JobRunner.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
